...
void ExtractTest::testExtraction_data()
{
    ...
    QTest::newRow("extract the whole simplearchive.tar.gz") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 4;
    ...
    QTest::newRow("extract the whole one_toplevel_folder.zip") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 9;
    ...
    QTest::newRow("extract the whole one_toplevel_folder.7z") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 9;
    ...
    QTest::newRow("extract selected entries from a 7z, drag-and-drop")
        << archivePath << QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("A/B/test2.txt"), QStringLiteral("A/B/"))} << dragAndDropOptions
        << 1;

    archivePath = QFINDTESTDATA("data/empty_folders.zip");
    QTest::newRow("zip with empty folders") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 5;

    archivePath = QFINDTESTDATA("data/empty_folders.tar.gz");
    QTest::newRow("tar with empty folders") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 5;
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.bz2");
    QTest::newRow("extract all entries from a bzip2-compressed tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.xz");
    QTest::newRow("extract all entries from a xz-compressed tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.lzma");
    QTest::newRow("extract all entries from a lzma-compressed tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;
    ...
    QTest::newRow("extract all entries from a compress (.Z)-compressed tarball with path")
        << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.lz");
    QTest::newRow("extract all entries from a lzipped tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;
    ...
    if (PluginManager().supportedMimeTypes().contains(QLatin1String("application/x-tzo"))) {
        ...
        QTest::newRow("extract the single-file test.png.lzo") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;
    }
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("lrzip")).isEmpty()) {
        ...
        QTest::newRow("extract the single-file test.txt.lrz") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;
    } else {
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("zstd")).isEmpty()) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.zst");
        QTest::newRow("extract all entries from a zst-compressed tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;

        archivePath = QFINDTESTDATA("data/test.txt.zst");
        QTest::newRow("extract the single-file test.txt.zst") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;
    } else {
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("lz4")).isEmpty()) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.lz4");
        QTest::newRow("extract all entries from a lz4-compressed tarball with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;

        archivePath = QFINDTESTDATA("data/test.txt.lz4");
        QTest::newRow("extract the single-file test.txt.lz4") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;
    } else {
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.xar");
    QTest::newRow("extract all entries from a xar archive with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 6;

    archivePath = QFINDTESTDATA("data/hello-1.0-x86_64.AppImage");
    QTest::newRow("extract all entries from an AppImage with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 7;

    archivePath = QFINDTESTDATA("data/hello-1.0-x86_64.AppImage");
    QTest::newRow("extract selected entries from an AppImage with path")
        << archivePath << QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("usr/bin/hello"), QString())} << optionsPreservePaths << 3;

    archivePath = QFINDTESTDATA("data/archive-multivolume.7z.001");
    QTest::newRow("extract all entries from a multivolume 7z archive with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 3;

    archivePath = QFINDTESTDATA("data/archive-multivolume.part1.rar");
    QTest::newRow("extract all entries from a multivolume rar archive with path") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 3;
    ...
    QTest::newRow("extract selected entries from rpm with path")
        << archivePath << QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("boot/sunxi-spl.bin"), QString())} << optionsPreservePaths << 2;

    archivePath = QFINDTESTDATA("data/firmware-pine64-20160329-6.1.aarch64.rpm");
    QTest::newRow("#369535: broken drag-and-drop from rpm")
        << archivePath
        << QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("boot/sunxi-spl.bin"), QStringLiteral("boot/")),
    ...
    QTest::newRow("extract the whole one_toplevel_folder.arj") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 9;
    ...
    QTest::newRow("extract the single-file test.z") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.zz");
    QTest::newRow("extract the single-file test.zz") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.txt.gz");
    QTest::newRow("extract the single-file test.txt.gz") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.txt.bz2");
    QTest::newRow("extract the single-file test.txt.bz2") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.png.lzma");
    QTest::newRow("extract the single-file test.png.lzma") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.svgz");
    QTest::newRow("extract the single-file test.svgz") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;

    archivePath = QFINDTESTDATA("data/test.sit");
    QTest::newRow("extract the whole test.sit") << archivePath << QVector<Archive::Entry *>() << optionsPreservePaths << 1;
